...
(function () {
  ...
  function start() {
    ...
    container.innerHTML = `
    <div id="wqs-req-header" style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:rgba(31,41,55,0.95);border-bottom:1px solid rgba(255,255,255,0.08)">
      <strong id="wqs-req-title" style="font-size:12px;color:#F3F4F6;white-space:nowrap;">API 请求监视器 · api.worldquantbrain.com</strong>
      <button id="wqs-req-count" title="点击展开 API 请求监视器" aria-label="展开 API 请求监视器" style="margin-left:auto;background:#374151;color:#E5E7EB;border:0;padding:3px 8px;border-radius:999px;cursor:pointer;font:inherit;">0</button>
      <button id="wqs-req-collapse" title="收起" aria-label="收起 API 请求监视器" style="background:#4B5563;color:#F9FAFB;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;">收起</button>
    ...
    function render() {
      ...
      for (let i = records.length - 1; i >= 0; i--) {
        ...
        const bodyBtnHtml = hasBody
          ? `<button class="wqs-body-btn" data-body="${encodeURIComponent(
              r.body.slice(0, 4000)
            )}" title="点击复制，悬浮预览" style="margin-left:6px;background:#374151;color:#E5E7EB;border:none;border-radius:4px;padding:2px 6px;cursor:pointer;">Body</button>`
          : "";

        tr.innerHTML = `
        <td style="padding:6px 10px;color:${statusColor}">${
          r.type === "completed" ? r.statusCode : "→"
        }</td>
        <td style="padding:6px 10px;color:#E5E7EB">${r.method || ""}</td>
  <td style="padding:6px 10px;max-width:260px;word-break:break-all;color:#D1D5DB">${(
    r.url || ""
  ).replace(/^[a-z]+:\/\//i, "")}${
          r._dupCnt && r._dupCnt > 1
            ? ' <span class="wqs-dup" style="color:#9CA3AF">×' +
              r._dupCnt +
              "</span>"
            : ""
        }${bodyBtnHtml}</td>
    ...
    function applyCollapsedState() {
      ...
      container.style.width = collapsed
        ? "auto"
        : "min(420px, calc(100vw - 24px))";
      countTag.title = collapsed ? "点击展开 API 请求监视器" : "当前请求数量";
      countTag.setAttribute(
        "aria-label",
        collapsed ? "展开 API 请求监视器" : "当前请求数量"
      );
    }
    ...
    function isExcluded(url) {
      if (!url) return false;
      return (
        Array.isArray(EXCLUDED_PREFIXES) &&
        EXCLUDED_PREFIXES.some(
          (p) => typeof p === "string" && url.startsWith(p)
        )
      );
    }
    ...
    chrome.runtime.onMessage.addListener((msg) => {
      if (
        msg &&
        msg.type === "REQ_MONITOR_NEW" &&
        msg.data &&
        String(msg.data.url).includes("api.worldquantbrain.com")
      ) {
        pushRecord(msg.data);
      }
    });
