export const BRAIN_OPS = new Set([
...
export const BRAIN_FIELDS = new Set([
...
export const BRAIN_API = new Set([
...
export function looksLikeFastExpr(text) {
    const value = String(text || '');
...
export function highlightFastExpr(text) {
    const re = /("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')|(\b\d+(?:\.\d+)?\b)|(\b[A-Za-z_][A-Za-z0-9_]*\b)|([^A-Za-z0-9_"']+)/g;
    let out = '';
    let match;
    while ((match = re.exec(String(text || '')))) {
        if (match[1]) {
            out += `<span class="hljs-string">${escapeHtml(match[1])}</span>`;
        } else if (match[2]) {
            out += `<span class="hljs-number">${match[2]}</span>`;
        } else if (match[3]) {
            const id = match[3];
            if (BRAIN_OPS.has(id)) out += `<span class="wqp-ce-op">${id}</span>`;
            else if (BRAIN_FIELDS.has(id)) out += `<span class="wqp-ce-field">${id}</span>`;
            else if (BRAIN_API.has(id)) out += `<span class="wqp-ce-api">${id}</span>`;
            else out += escapeHtml(id);
        } else {
            out += escapeHtml(match[4]);
        }
    }
    return out;
}

export function classifyInlineCode(word) {
...
export function authorIdFrom(text) {
    const value = String(text || '')
        .replace(/[★☆]/g, '')
        .trim()
        .split(/\s+/)[0];
...
export function normalizeFollowedIds(raw) {
